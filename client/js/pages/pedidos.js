      ...
      ${data.map(pedido => {
        ...
        <a href="#/pedidos/${pedido.id}" class="pedido-card" style="display:flex;align-items:center;gap:var(--space-md)">
          <div style="width:56px;height:56px;border-radius:10px;overflow:hidden;background:var(--bg-secondary);flex-shrink:0">
            ${imgUrl ? `<img src="${imgUrl}" alt="" style="width:100%;height:100%;object-fit:contain;padding:4px" onerror="this.onerror=null;this.parentElement.innerHTML='📦'" />` : `<div style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;font-size:1.2rem;color:var(--text-tertiary)">📦</div>`}
          </div>
